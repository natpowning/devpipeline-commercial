...
resource "aws_instance" "docker-dtr" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker run --rm docker/dtr install --ucp-node ${aws_instance.docker-dtr.private_dns} --ucp-username ${var.admin_username} --ucp-password ${var.admin_password} --ucp-url https://${aws_instance.docker-ucp.0.private_ip} --ucp-ca \"$(cat /tmp/ucp_ca)\""
    ]
  }
