...
resource "aws_instance" "docker_dtr_first" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker run --rm docker/dtr install --ucp-node ${aws_instance.docker_ucp_first.private_dns} --ucp-username ${var.admin_username} --ucp-password ${var.admin_password} --ucp-url https://${aws_instance.docker_ucp_first.private_ip} --ucp-ca \"$(cat /tmp/ucp_ca)\"",
    ]
  }
